...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr te3DCanvas::Segment(const std::vector<QPointF>& pointSet)
{
    if (cloud->empty()) {
...
void te3DCanvas::CloudToMarkers(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_in)
{
    if(cloud_in != nullptr)
    {
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> currentColor(cloud_in, m_curstate.currentColor.red(), m_curstate.currentColor.green(), m_curstate.currentColor.blue());
        QString CloudId;
        QString test = m_curstate.currentCategory;
        auto it = markerPCID.find(m_curstate.currentCategory);
        auto pct = markerPointCloud.find(m_curstate.currentCategory);
        if (it != markerPCID.end()) {
            QString count = incrementNumber(it->second.back());
            CloudId = m_curstate.currentCategory + QString::number(m_curstate.currentIndex) + "marker" + count;
            it->second.push_back(CloudId);
            pct->second.push_back(cloud_in);
        }
        else {
            CloudId = m_curstate.currentCategory + QString::number(m_curstate.currentIndex) + "marker" + "0";
            markerPCID.insert(std::make_pair(m_curstate.currentCategory, std::vector<QString>{CloudId}));
            markerPointCloud.insert(std::make_pair(m_curstate.currentCategory, std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr>{cloud_in}));
        }
        viewer->addPointCloud(cloud_in, currentColor, CloudId.toStdString());
        ...
        emit sig_3DCanvasMarkingCompleted(cloud_in);
...
void te3DCanvas::UpdateDimentsion()
{
    ...
    for (itStr, itPC; itStr != markerPCID.end(); ++itStr, ++itPC)
    {
        ...
        {
            pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> currentColor(itPC->second.at(i), m_curstate.currentColor.red(), m_curstate.currentColor.green(), m_curstate.currentColor.blue());
...
bool te3DCanvas::CoordinateAxisRendering(QString curaxis)
{
    ...
    if (curaxis == "z")
    {
        min = m_curstate.currentInvalidPointThreshold;
        max = m_curstate.currentValidPointThreshold;
        int factor = max - min;
...
void te3DCanvas::MarkersShowInCanvas(te::AiInstance* instance, cv::Mat& m_image,QColor color)
{
    ...
    if (it != markerPCID.end()) {
        ...
        CloudId = QString::fromStdString(instance->name) + QString::number(m_curstate.currentIndex) + "marker" + "0";
        markerPCID.insert(std::make_pair(m_curstate.currentCategory, std::vector<QString>{CloudId}));
        markerPointCloud.insert(std::make_pair(m_curstate.currentCategory, std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr>{cloud_marked}));
    }
...
void te3DCanvas::ResultsShowInCanvas(te::AiInstance* instance, cv::Mat& m_image, QColor color)
{
    ...
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> currentColor(cloud_marked, color.red(), color.green(), color.blue());
    QString CloudId;
    auto it = resultPCID.find(m_curstate.currentCategory);
    auto pct = resultPointCloud.find(m_curstate.currentCategory);
    if (it != resultPCID.end()) {
        QString count = incrementNumber(it->second.back());
        CloudId = QString::fromStdString(instance->name) + QString::number(m_curstate.currentIndex) + "result" + count;
        it->second.push_back(CloudId);
        pct->second.push_back(cloud_marked);
    }
    else {
        CloudId = QString::fromStdString(instance->name) + QString::number(m_curstate.currentIndex) + "result" + "0";
        resultPCID.insert(std::make_pair(m_curstate.currentCategory, std::vector<QString>{CloudId}));
        resultPointCloud.insert(std::make_pair(m_curstate.currentCategory, std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr>{cloud_marked}));
    }
    viewer->addPointCloud(cloud_marked, currentColor, CloudId.toStdString());
...
void te3DCanvas::GuassFilter(QString data1, QString data2, QString data3, QString data4)
{
    if (cloud->empty()) {
        ...
        pcl_filter_guass(cloud, cloud_Filter_out, data1.toFloat(), data2.toFloat(), data3.toFloat(), data4.toFloat());
...
void te3DCanvas::pcl_filter_guass(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_out, float paraA, float paraB, float paraC, float paraD)
{
...
void te3DCanvas::CrossSection(QString data1, QString data2, QString data3, QString data4)
{
    if (cloud->empty()) {
        QMessageBox::warning(this, "Warning", u8"无点云输入");
        return;
        ...
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_Filter_out = (new pcl::PointCloud<pcl::PointXYZRGB>())->makeShared();
        pcl_crossSection(cloud, cloud_Filter_out, data1.toFloat(), data2.toFloat(), data3, data4.toFloat());
        pcl::copyPointCloud(*cloud_Filter_out, *cloud);
...
void te3DCanvas::pcl_crossSection(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_out, float min, float max, QString axis, float is_save)
{
...
void te3DCanvas::CurrentStateChanged(const QString& category, const QColor& fontColor, const int& index, const double& valThreshold, const double& invalThreshold)
{
...
void te3DCanvas::te3DCanvasStartMarking(QVector<QPointF>& pointlist)
{
    ...
    CloudToMarkers(Segment(pointSet));
}
