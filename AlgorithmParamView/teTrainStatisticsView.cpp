...
TrainStatisticsView::TrainStatisticsView(QWidget* parent)
	: QWidget(parent)
	, ui(new Ui::TrainStatisticsViewClass())
{
	ui->setupUi(this);
	...
	initQPlot(customPlot);
}

TrainStatisticsView::~TrainStatisticsView()
{
	delete ui;
}

void TrainStatisticsView::ReceiveData()
{
	TrainStatisticsViewMenber menber = viewModel.lock()->getTrainStateMenber();
	showPlot(customPlot, menber.iteration, menber.fAvgLoss, menber.fPosAcc);
}

void TrainStatisticsView::on_iteration_checkBox_stateChanged(int arg)
{
	if (arg) {
...
void TrainStatisticsView::on_fAvgLoss_checkBox_stateChanged(int arg)
{
	if (arg) {
...
void TrainStatisticsView::on_fPosAcc_checkBox_stateChanged(int arg)
{
	if (arg) {
...
void TrainStatisticsView::isShow(int arg)
{
	if (arg > 0) {
		show();
	}
...
void TrainStatisticsView::restoreInitialState()
{
...
void TrainStatisticsView::initQPlot(QCustomPlot* customPlot)
{
	// 图表添加两条曲线
...
void TrainStatisticsView::showPlot(QCustomPlot* customPlot, int iterationNum, float fAvgLossNum, float fPosAccNum)
{
	static double cnt;
...
void TrainStatisticsView::closeEvent(QCloseEvent* event)
{
	viewModel.lock()->notified(ViewModel::UnCheckTrainBox);
	event->accept();
}

void TrainStatisticsView::handlePositionToolTip(QMouseEvent* event)
{
...
bool TrainStatisticsView::eventFilter(QObject* watched, QEvent* event)
{
...
void te::TrainStatisticsView::bindViewModel(std::shared_ptr<ViewModel> vm)
{
	viewModel = vm;
	if (viewModel.lock())
	{
		connect(viewModel.lock().get(), &ViewModel::notified, this, &TrainStatisticsView::refresh);
	}
}

void te::TrainStatisticsView::refresh(ViewModel::updateMode mode)
{
	if (mode == ViewModel::TrainStatistics)
	{
		ReceiveData();
	}
	else if (mode == ViewModel::ShowTrainStateChart)
	{
		int arg = viewModel.lock()->getTrainStatisticsState();
		isShow(arg);
	}
}
