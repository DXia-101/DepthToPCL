...
void teLabelBrowser::displayUIInWidget(QVBoxLayout* layout)
{
...
void teLabelBrowser::updateTrainWidget(QMap<QString, int>* nameCounts)
{
	for (int row = 0; row < LabelWidget->rowCount(); ++row)
	{
...
void teLabelBrowser::updateResultWidget(QMap<QString, int>* nameCounts)
{
	for (int row = 0; row < LabelWidget->rowCount(); ++row)
	{
		QTableWidgetItem* nameItem = LabelWidget->item(row, 0);
	...
	if (selectedItem) {
		...
		if (colorDialog.exec() == QDialog::Accepted) {
			...
			if (firstColumnItem) {
				...
				emit sig_ColorChanged(selectedColor);
			}
		}
	}
